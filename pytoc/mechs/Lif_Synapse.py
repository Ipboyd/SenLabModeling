...
import numpy as np
def Build_Vars(ESYN = 0, tauD = 1.5, tauR = 0.3, delay = 0, gSYN = 1, fF = 0, fP = 0, tauF = 180, tauP = 60, maxF=4 , name=''):
    ...
    #Build dictionary
    return {f'{name}_ESYN' : ESYN,f'{name}_tauD' : tauD,f'{name}_tauR' : tauR,f'{name}_delay' : delay,f'{name}_gSYN' : gSYN,f'{name}_fF' : fF,f'{name}_fP' : fP,f'{name}_tauF' : tauF,f'{name}_tauP' : tauP,f'{name}_maxF' : maxF , 'name' : name}
